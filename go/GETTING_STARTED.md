# Getting Started with Game of Life kata in Go
...
> ***Reminder***: the commands below should be run from [Kata-GameOfLife/go]() directory
...
> ***Reminder***: the command below should be run from [Kata-GameOfLife/go]() directory
...
`File` > `Open` > `Kata-GameOfLife` > `go`

Run all the tests in [game_of_life_test.go](./src/game_of_life/game_of_life_test.go).
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-GameOfLife` / `go`
