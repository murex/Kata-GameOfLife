# Getting Started with Game of Life kata in Python
...
> ***Reminder***: the commands below should be run from [Kata-GameOfLife/python](.) directory
...
`File` > `Open` > `Kata-GameOfLife` > `python`
...
> ***Reminder***: the command below should be run from [Kata-GameOfLife/python](.) directory
...
Choose `Open Folder`, navigate to `Kata-GameOfLife` / `python` then click `Select Folder`.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-GameOfLife` / `python`
