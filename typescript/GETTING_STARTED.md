# Getting Started with Game of Life kata in Typescript
...
`File` > `Open` > `Kata-GameOfLife` > `typescript`
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-GameOfLife` / `typescript`
